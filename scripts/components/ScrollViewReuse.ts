...
export class ScrollViewReuse extends cc.Component {
        ...
        for (let i = 0; i < this._items.length; ++i) {
            ...
            switch (this.align) {
                case eScrollViewReuseAlign.HORIZONTAL:
                    this._updateHorizontal(itemInViewPos, i);
                    break;
                case eScrollViewReuseAlign.VERTICAL:
                    this._updateVertical(itemInViewPos, i);
                    break;
                default:
                    break;
            }
    ...
    private _updateCellInfoByIndex(index: number, svIndex?: number, disCount?: number): void {
        ...
        if (item_component) {
            ...
            if (typeof item_component.updateSVReuseData === 'function')
                item_component.updateSVReuseData(item_component.svIndex, this._itemsData, this, index);
        }
    }
    ...
    updateCellAtIndex(index: number): void {
        for (let i = 0; i < this._items.length; ++i) {
            ...
            if (item_component && item_component.svIndex === index) {
                if (typeof item_component.updateSVReuseData === 'function')
                    item_component.updateSVReuseData(item_component.svIndex, this._itemsData, this, i);
            }
        }
    }
    ...
    onDisable() {
        if (
            ...
            if (offset < 0) {
                ...
                this.scrollView.setContentPosition(
                    cc.v2(0, this.scrollView.content.height - this.scrollView.node.height * 0.5)
                );
            }
        }
    }
