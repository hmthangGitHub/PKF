import { LANGUAGE_GROUPS, languageManager, MathUtil } from 'pf';
...
export class StringUtil {
    ...
    static clientGoldByServer(gold: number): number {
        return MathUtil.times(gold, 0.01);
    }
    ...
    static serverGoldByClient(gold: number): number {
        return MathUtil.times(gold, 100);
    }
    ...
    static numberToShowNumber(number: number): number {
        return MathUtil.times(StringUtil._getShowGoldRatio(), number);
    }
    ...
    static showStringToNumber(str: string): number {
        return MathUtil.div(parseFloat(str), StringUtil._getShowGoldRatio());
    }
    ...
    static _getShowGoldRatio(): number {
        ...
        const number = 100;
        return MathUtil.div(100, number);
    }
    ...
    static numberToString(number: number): string {
        ...
        if (number === 0) {
            ...
            if (number * 10 - parseInt((fnum * 10).toString(), 10) > 0) {
                result = MathUtil.floorToDecimalPlaces(number, 2);
            } else if (number - parseInt(number.toString(), 10) === 0) {
                result = number;
            } else {
                result = MathUtil.floorToDecimalPlaces(number, 1);
            }
            return StringUtil.numberToShowString(result);
        } else {
            let newNum = -number;
            let fnum = parseFloat(newNum.toString());
            if (newNum - parseInt((fnum * 10).toString(), 10) > 0) {
                result = MathUtil.floorToDecimalPlaces(newNum, 2);
            } else if (newNum - parseInt(newNum.toString(), 10) === 0) {
                result = newNum;
            } else {
                result = MathUtil.floorToDecimalPlaces(newNum, 1);
            }
            return StringUtil.numberToShowString(-result);
        }
    }
    ...
    static getArrayLength(value: any): number {
        ...
        if (value !== null && typeof value !== 'undefined' && Array.isArray(value)) {
            retVal = Number(value.length);
        }
        return retVal;
    }

    static getSignString(value: number): string {
        let sRet: string = value.toString();
    ...
    static getServerStrByLanguage(strData: string): string {
        ...
        let index = this.getLanguageIndex();
